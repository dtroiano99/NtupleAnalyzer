...
void Subtraction(TH1F* h1,  int n){
...
void QCD_m8(){
  ...
  */
  ...
  TString fname= "./Plots/Fit_Data20_m8.pdf";
  auto canv = new TCanvas("Canvas", "Canvas", 1500, 600);
  canv->cd();
  canv->Print(fname+"[");

  ofstream outfile;
  outfile.open ("results_m8.txt", std::ofstream::out | std::ofstream::trunc);

  for(int j=0; j<functions.size();j++){
    cout<<functions.at(j)<<endl;
    std::string filestring1{"rootfiles/Fit_"+functions.at(j)+"20_m8.root"};
