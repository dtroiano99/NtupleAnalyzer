...
void QCD_2(){
  ...
  int bin1 = 56;
  int l[]={70, 62, 78, 54, 86, 62, 78, 70, 70};
  int u[]={126,134,118,142,110,126,126,134,118};
  ...
  for(int i=0;i<rebinning.size();i++){
    ...
    for(int z=0; z< (sizeof(l) / sizeof(l[0]));z++){
      ...
      TFile* file1;
      std::string filestring{"./rootfiles/Fit_Cebysev20_"+std::to_string(l[z])+"_"+std::to_string(u[z])+".root"};
      ...
      //float ErryQCD =0;
      //cout<<l[z]<<"_"<<u[z]<<"   "<<yQCD<<"  "<<ErryQCD<<endl;
      r[z] = r1;
    ...
    for(int k=1; k< QCD->GetNbinsX()+1;k++){
      ...
      //float yQCD0 = (f->Integral(xData-(xwidth/2),xData+(xwidth/2)))/xwidth;
