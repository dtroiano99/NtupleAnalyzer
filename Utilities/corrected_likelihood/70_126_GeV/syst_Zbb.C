...
void syst_Zbb(){
  ...
  for(int iPN=0; iPN<PN_DIM;iPN++){
    ...
    for (map<string, double>::iterator i = AK8PuppiJets_Lpt_softdropmass_up->begin(); i != AK8PuppiJets_Lpt_softdropmass_up->end(); i++) {
                        ...
                        PN_Z_u[iPN][k]->GetXaxis()->SetTitle("m_{SD} / 200 GeV");    
                        ...
                        PN_Z_d[iPN][k]->GetXaxis()->SetTitle("m_{SD} / 200 GeV");    
  ...
  for(int i=0; i<PN_DIM;i++){
    for(int k=0; k< SYS_DIM; k++){
      PN_Z_d[i][k]->Rebin(rebinning.at(i)); 
      PN_Z_u[i][k]->Rebin(rebinning.at(i)); 
      PN_Z_d[i][k]->Write();
      PN_Z_u[i][k]->Write();
    }
  }
