...
void Histo_Wjets(){
  ...
  for(int iPN=0; iPN<PN_DIM;iPN++){
    ...
    PN_Data[iPN]->GetXaxis()->SetTitle("m_{SD} / 200 GeV");    
  ...
  for (int i = 0; i < nentries; i++){      
	...
	if(AK8_sdmJet0>l && AK8_sdmJet0<u ){
	  PN_Data_s[iPN]->Fill(AK8_sdmJet0/s,lumiWeight);	  
	}

	if((AK8_sdmJet0>Start && AK8_sdmJet0<l) || (AK8_sdmJet0 >u && AK8_sdmJet0<End)){
	  PN_Data[iPN]->Fill(AK8_sdmJet0/s,lumiWeight);
	} 
      }
    }
  ...
  TFile *myfile;
  myfile = TFile::Open("Wjets_histos.root","RECREATE");
  myfile->cd();
