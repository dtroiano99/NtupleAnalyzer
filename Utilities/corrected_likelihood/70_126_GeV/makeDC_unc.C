...
void makeDC_PN(int bin = 0){
  cout<<bin<<endl;
  TFile *outFile;
  std::string rootstring{"/lustre/home/dtroiano1/CMSSW_10_6_12/src/Zqq/NtupleAnalyzer/Utilities/corrected_likelihood/70_126_GeV/rootfiles/Bin"+std::to_string(bin)+"_unc.root"};
  ...
  std::string datastring{"PNhist"+std::to_string(bin)+"_s;1"};
  ...
  std::string MCstring{"PNhist"+std::to_string(bin)+"_s;2"};
  ...
  */
  ...
  for(int v=0; v<variation.size();v++){
    for(int s=0; s<sources.size();s++){
      ...
      cout<<sources.at(s)<<"   "<<variation.at(v)<<"   "<<100*(Zbb_syst->Integral(1,Zbb_syst->GetNbinsX())-MCI)/MCI<<"%"<<endl;
      if(v==0){
	datacard << left<<setw(20)<<sources.at(s)+"_s"+std::to_string(bin)<< setw(15) <<"shape"<< setw(15)<< "1"<< setw(15) << "-"  << endl;
      }
      outFile->cd();
      Zbb_syst->Write();
