...
//static const std::vector<float> PN_scores0 {0.641, 0.875};
...
void Fit_Data20(){
  ...
  for(int iPN=0; iPN<PN_scores0.size()-1;iPN++){
    ...
    if(iPN%2 ==0){
      ...
      PN_Data[i]->GetXaxis()->SetTitle("m_{SD}/ 250 GeV");
    }
    else{
      int i = (int)(iPN-1)/2;
  ...
  //draw raw histograms
 /* 
  TString fname1= "./Plots/Data_sb.pdf";
  ...
  */
  ...
  TString fname= "./Plots/Fit_Data20.pdf";
  auto canv = new TCanvas("Canvas", "Canvas", 1500, 600);  
  canv->cd();
  canv->Print(fname+"[");
  ...
  for(int j=0; j<functions.size();j++){
    ...
    std::string filestring1{"rootfiles/Fit_"+functions.at(j)+"20.root"};
