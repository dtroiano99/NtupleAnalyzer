...
{{4,2,3,3}};//indexes of Cebysev polynomials    
/*
static const std::vector<vector<float>> syst
{{0,0,0,0}};//systematics of Cebysev polynomials
*/
static const std::vector<vector<float>> syst
{{1057.67,226.986,300.443,242.184}};//systematics of Cebysev polynomials
...
void QCD_DDX_Data3(){
  ...
  */
  ...
  TString fname= "./Plots/comparison_DDX2.pdf";
  auto canv = new TCanvas("Canvas", "Canvas", 1500, 600);  
  canv->cd();
  canv->Print(fname+"[");
  ...
  for(int j=0; j<functions.size();j++){
    ...
    for(int i=0;i<DDX_DIM;i++){   
      ...
      std::string filestring{"./rootfiles/DDX_"+ functions.at(j)+"2.root"};
  ...
  TString fname1= "./Plots/DDXScore3.pdf";
  ...
  for(int i=0;i<functions.size();i++){       
    ...
    p1->cd();
    auto legend1 = new TLegend(0.6,0.58,0.85,0.89);    
    ...
    //p1->SetLogy();    
    score_MC->GetYaxis()->SetTitle("Events");  
    score_MC->SetMaximum(2600);
    score_MC->SetMinimum(600);
    ...
    hShadow->GetYaxis()->SetRangeUser(0.6,2.0);
  ...
  /*
   //draw score histograms
  TLatex tex1; tex1.SetNDC();
  TString fname1= "./Plots/DDXScore2.pdf";
  auto canv1 = new TCanvas("Canvas1", "Canvas1", 2000, 1500);  
  canv1->cd();
  canv1->Print(fname1+"[");
  for(int i=0;i<functions.size();i++){       
