...
void Fit_DDX_Data2_1(){
  ...
  TFile *f;
  f = TFile::Open("Data_histos_1.root","read");
  ...
  //draw raw histograms
  /*
  TString fname1= "./Plots/Hist2_1.pdf";
  ...
  */
  ...
  TString fname= "./Plots/DDX_Fit2_1.pdf";
  auto canv = new TCanvas("Canvas", "Canvas", 1500, 600);  
  canv->cd();
  canv->Print(fname+"[");

  ofstream outfile;
  outfile.open ("results_1.txt", std::ofstream::out | std::ofstream::trunc);

  for(int j=0; j<functions.size();j++){
    cout<<functions.at(j)<<endl;
    std::string filestring1{"rootfiles_1/DDX_"+functions.at(j)+"2.root"};
    ...
    for(int i=0;i<DDX_DIM;i++){   
	 ...
	 if(j==0){tex.DrawLatex(0.7,0.4,"#scale[0.8]{Polynomial}");}
	 if(j==1){tex.DrawLatex(0.7,0.4,"#scale[0.8]{Cebysev polynomial}");}
	 if(j==2){tex.DrawLatex(0.7,0.4,"#scale[0.8]{Dijet family function}");}
