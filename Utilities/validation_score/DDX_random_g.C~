...
void random_g(){
  ...
  ofstream outfile;
  outfile.open ("toys.txt", std::ofstream::out | std::ofstream::trunc);

  TFile *myfile;
  myfile = TFile::Open("rootfiles/toys.root","RECREATE");

  myfile->cd();
  //create directories in each root file for each order of the fitting function
  for(int ju=0; ju<idx.size(); ju++){
    std::string dirstring{"PN_bin"+std::to_string(ju)};
  ...
  for(int j=0;j<idx.size();j++){
    ...
    std::string dirstring{"PN_bin"+std::to_string(j)};
    const char * dirname{dirstring.c_str()};
    myfile->cd(dirname);
    ...
    outfile<<"PN-MD_BBvsQCD bin"<<j<<endl;
