...
static const std::vector<string> functions {"pol","Cebysev","DF"};

static const std::vector<float> DDX_scores0 {0.022193,0.0502867,0.0939224,0.153532, 0.238822,0.356848,0.527736,0.738001,1};

static const std::vector<float> DDX_scores {0.022193,0.0939224, 0.238822,0.527736,1};

int DDX_DIM =  DDX_scores.size()-1; //number of DDX score bins

static const std::vector<vector<int>> idx 
{{3,3,4,2},//indexes of Polynomials    
  {3,3,4,2},//indexes of Cebysev polynomials    
    {2,2,6,2}}; //indexes of DF
...
void drawratio(TCanvas *C, TH1F *MC, TH1F *DD, TLegend *legend,  int u, int j){
  ...
  DD->Draw("PEsame");
  legend->Draw();
  if(j==0){tex.DrawLatex(0.25,0.8,"#scale[1]{Polynomial}");}
...
void QCD_DDX_Data2_1(){
  ...
  TH1F *PN_Data[DDX_DIM];
  
  for(int iPN=0; iPN<DDX_scores0.size()-1;iPN++){
    std::string nomestringa{"DDXhist"+std::to_string(iPN)+"_s"};
    const char * nome{nomestringa.c_str()};
    if(iPN%2 ==0){
      int i = (int)iPN/2;
      PN_Data[i] = (TH1F*)f1->Get(nome);
      float start = floor(DDX_scores.at(i) * 10000) / 10000;
      std::stringstream ss1;
      ss1 <<start;
      std::string str1 = ss1.str();
      float end = floor(DDX_scores.at(i+1) * 10000) / 10000;
  ...
  */
  ...
  //DDX_BBvsQCD score histograms

  TH1F *score_MC; TH1F *score_QCD[functions.size()]; TH1F *score_DATA;

  const Int_t NBINS = DDX_DIM;
  Double_t edges[NBINS+1];
  for(int iPN=0; iPN<DDX_DIM+1;iPN++){
    edges[iPN]=DDX_scores.at(iPN);    
  }
  score_MC= new TH1F("DDX_MC","",NBINS,edges);
  score_DATA= new TH1F("DDX_DATA","",NBINS,edges);
  score_MC->GetXaxis()->SetTitle("DDX_BBvsQCD score"); 
  score_DATA->GetXaxis()->SetTitle("DDX_BBvsQCD score");
  ...
  TString fname= "./Plots/comparison_DDX2_1.pdf";
  auto canv = new TCanvas("Canvas", "Canvas", 1500, 600);  
  canv->cd();
  canv->Print(fname+"[");
  ...
  for(int j=0; j<functions.size();j++){
    ...
    score_QCD[j]= new TH1F("DDX_QCD",SCname,NBINS,edges);
    score_QCD[j]->GetXaxis()->SetTitle("DDX_BBvsQCD score");
    ...
    for(int i=0;i<DDX_DIM;i++){   
      ...
      std::string filestring{"./rootfiles_1/DDX_"+ functions.at(j)+"2.root"};
      ...
      std::string rstring{std::to_string(idx.at(j).at(i))+"parameters/TFitResult-DDXhist"+std::to_string(i*2)+"-fun"+std::to_string(i)};
	...
	score_DATA->SetBinError(i+1,valueErrorDATA); 
      ...
      myfile << "score region #"<<i<<": Data = "<<valueDATA<<" || MC Zqq = "<< valueMC<<"#pm"<<valueErrorMC<<" || QCD = "<<QCD_<<"#pm"<<sqrt(e_QCD_)<<" || Data-driven Zqq = "<<PN_Data[i]->Integral()-QCD_<<"#pm"<<sqrt(e_DDZqq_)<<" ||total QCD = "<<yQCD<<"#pm"<<sqrt(erryQCD)<<  endl;
  ...
  TString fname1= "./Plots/DDXScore2_1.pdf";
  ...
  for(int i=0;i<functions.size();i++){       
    ...
    p1->cd();
    auto legend1 = new TLegend(0.71,0.58,0.82,0.89);
    ...
    hShadow->GetYaxis()->SetRangeUser(0.64,1.03);
