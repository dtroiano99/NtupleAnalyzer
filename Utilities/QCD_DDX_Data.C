...
{{3,5,3},//indexes of polynomials
    {3,3,3}, //indexes of Cebysev polynomials
      {3,3,3}, //indexes of DF
...
void drawratio(TCanvas *C, TH1F *MC, TH1F *DD, TLegend *legend,  int u){
  ...
  DD->Draw("PEsame");
  legend->Draw();
  tex.DrawLatex(0.75,0.92,"#scale[1]{39.2 fb^{-1} (13.6 TeV)}");
...
void QCD_DDX_Data(){
  ...
  TString fname= "./Plots/comparison_DDX.pdf";
  ...
  for(int j=0; j<functions.size();j++){
    ...
    for(int i=0;i<DDX_DIM;i++){   
      ...
      std::string filestring{"./rootfiles/DDX_"+ functions.at(j)+".root"};
	...
	DDX_MC[ij]->SetTitle(title);
      ...
      drawratio(canv,DDX_MC[ij], Zqq_dd[ij],legend,   ij);
