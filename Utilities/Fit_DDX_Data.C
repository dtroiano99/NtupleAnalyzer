...
void Fit_DDX_Data(){
  ...
  for(int j=0; j<functions.size();j++){
    ...
    for(int i=0;i<DDX_DIM;i++){   
	 ...
	 if(j==4){function = PPF(jOR);}
	 if(j==5){function = PEF(jOR);}
	 if(j==6){function = UA2F(jOR);}
	 ...
	 if(j==3){tex.DrawLatex(0.7,0.4,"#scale[0.8]{Modified Dijet Family function}");}
	 if(j==4){tex.DrawLatex(0.7,0.4,"#scale[0.8]{Polynomial Power Family function}");}
	 if(j==5){tex.DrawLatex(0.7,0.4,"#scale[0.8]{Polynomial Extension Family function}");}
	 if(j==6){tex.DrawLatex(0.7,0.4,"#scale[0.8]{UA2 Family function}");}
